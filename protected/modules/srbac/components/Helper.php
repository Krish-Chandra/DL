...
class Helper {
  ...
  public static function getRoleAssignedTasks($name) {
	...
	public static function getRoleAssignedTasksRecursive($name) 
	{
		$childRolesCriteria = new CDbCriteria();
		$result = array();
		if($name)
		{
			$childRolesCriteria->condition = "parent='" . $name . "'";
			$result = ItemChildren::model()->findAll($childRolesCriteria);
		}
